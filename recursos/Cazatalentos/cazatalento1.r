...
for( base_tiros in c(10, 15, 20, 25, 30, 35) ){
  for( coeficiente in c(5,20,35,50,65,10) ){
    ...
    for( i in 1:10000 ) { #diez mil experimentos
      ...
      GLOBAL_tiros_total  <- 0
      tiros_r1 <- base_tiros 
      tiros_r2 <- (tiros_r1 + coeficiente)
      tiros_r3 <- (tiros_r2 + coeficiente)
      tiros_r4 <- (tiros_r3 + coeficiente)
      ...
      minmed  <- planilla_cazatalentos[ ids_juegan1, quantile(aciertos1, prob=seq(0, 1, length = 101))[16]]
      ...
      resultado3  <- gimnasio_tirar( ids_juegan4, tiros_r4)
    ...
    cat( tiros_r1 , tiros_r2, tiros_r3, tiros_r4, GLOBAL_tiros_total, tasa_de_aciertos/100, "\n" )
