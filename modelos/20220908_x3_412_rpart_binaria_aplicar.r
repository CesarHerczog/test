...
dataset$mpayroll[is.na(dataset$mpayroll)] <- 0
...
dtrain <- dtrain%>% mutate_each_ (list (~ scale (.)%>% as.vector),
                                  vars = c ("mpayroll","mcuentas_saldo","cdescubierto_preacordado","mprestamos_personales","mcaja_ahorro","mcuenta_corriente","mactivos_margen","ccomisiones_otras","mpasivos_margen","mtarjeta_visa_consumo","Visa_msaldototal","Visa_msaldopesos"))
