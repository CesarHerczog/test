...
dataset[ ,ctrx_quarter_bin :=  ifelse( ctrx_quarter < -1.1827, 0, 1 ) ]
dataset[ ,mcaja_ahorro_bin :=  ifelse( mcaja_ahorro < -0.36, 0, 1 ) ] 
...
dtrain.nafill        <- function(x) { nafill(dtrain, fill=0)} #fill NA con data.table porque es más rápido
dtrain <- dtrain%>% mutate_each_ (list (~ scale (.)%>% as.vector), #normalizo todos los valores desde el mes hasta ternaria por separado
                                  vars = c (3:154))


dapply  <- dataset[ foto_mes==202103, ]  #defino donde voy a aplicar el modelo
dapply.nafill        <- function(x) { nafill(dapply, fill=0)} #fill NA con data.table porque es más rápido
dapply[ foto_mes==202103, clase_ternaria := "" ]
dapply[ foto_mes==202103, clase_binaria :=  ifelse( clase_ternaria=="CONTINUA", "NO", "SI" ) ]
dapply <- dapply%>% mutate_each_ (list (~ scale (.)%>% as.vector), #normalizo todos los valores desde el mes hasta ternaria por separado
                                  vars = c (3:154))
...
modelo  <- rpart(formula=   "clase_binaria ~ .   -Visa_mpagado -mcomisiones_mantenimiento -clase_ternaria -ctrx_quarter -mcaja_ahorro",
...
for( corte  in  c( 8700, 10500 ) )
{
  ...
  fwrite( dfinal[ , list(numero_de_cliente, Predicted) ], #solo los campos para Kaggle
           file= paste0( "./exp/KA113111/20220905_Norm_KA113111_005_",  corte, ".csv"),
           sep=  "," )
}
