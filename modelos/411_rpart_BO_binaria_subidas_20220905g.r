...
hs  <- makeParamSet(
          makeNumericParam("cp"       , lower=  -0.079, upper=    -0.071),
          makeNumericParam("minsplit" , lower=   870,   upper= 930 ),
          makeNumericParam("minbucket", lower=   27,   upper= 33 ),
...
ArbolSimple  <- function( fold_test, data, param )
{
  ...
  #genero el modelo
  modelo  <- rpart("clase_binaria ~ . -mcaja_ahorro -Master_mpagado -Visa_mpagado -ctrx_quarter -mcomisiones_mantenimiento -clase_ternaria -Visa_status -Master_status",
...
archivo_log  <- "HT20220905g.txt"
archivo_BO   <- "HT20220905g.RDATA"
