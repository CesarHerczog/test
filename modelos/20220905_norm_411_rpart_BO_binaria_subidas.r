...
hs  <- makeParamSet(
          makeNumericParam("cp"       , lower=  -0.14, upper=    -0.04),
          makeNumericParam("minsplit" , lower=   730L,   upper= 1000L ),
          makeNumericParam("minbucket", lower=   29L,   upper= 75L ),
          makeIntegerParam("maxdepth" , lower=   7L,  upper=   9L),  #la letra L al final significa ENTERO
...
ArbolSimple  <- function( fold_test, data, param )
{
  ...
  #genero el modelo
  modelo  <- rpart("clase_binaria ~ .   -Visa_mpagado -mcomisiones_mantenimiento -clase_ternaria -Visa_status -Master_status",
...
dataset.nafill <- function(x) { nafill(dataset, fill=0)}
dataset <- dataset%>% mutate_each_ (list (~ scale (.)%>% as.vector), #normalizo todos los valores desde el mes hasta ternaria por separado
                                  vars = c (3:154))
...
archivo_log  <- "HT20220905x1.txt"
archivo_BO   <- "HT20220905x1.RDATA"
