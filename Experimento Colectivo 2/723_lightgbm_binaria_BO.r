...
PARAM$experimento  <- "Experimento_colectivo_2"

PARAM$input$dataset       <- "./datasets/exp_DR9141_dataset2.csv.gz"
...
PARAM$trainingstrategy$semilla_azar   <- 113111  #Aqui poner la propia semilla

PARAM$hyperparametertuning$iteraciones <- 30 #100
PARAM$hyperparametertuning$xval_folds  <- 5
PARAM$hyperparametertuning$POS_ganancia  <- 78000
PARAM$hyperparametertuning$NEG_ganancia  <- -2000

PARAM$hyperparametertuning$semilla_azar  <- 113111  #Aqui poner la propia semilla, PUEDE ser distinta a la de trainingstrategy
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  param_variable  <- list(  early_stopping_rounds= as.integer(10/x$learning_rate) )  #Original as.integer(50 + 5/x$learning_rate)
